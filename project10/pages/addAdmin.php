...
$elementsArr = [
  "masterStatus"=>[
	...
	"email"=>[
		"errorMessage"=>"<span style='color: red; margin-left: 15px;'>Must Be Valid Email</span>",
    "errorOutput"=>"",
    "type"=>"text",
...
function addData($post){
  ...
  /* IF EVERYTHING WORKS ADD THE DATA HERE TO THE DATABASE HERE USING THE $_POST SUPER GLOBAL ARRAY */
      ...
      $pdo = new PdoMethods();
      $sql = "SELECT email FROM adminMod Where email = :email";
      $bindings = [
      ...
      //$records = 2;
      if(count($records)>0){
        return getForm("<p>User already exists</p>", $elementsArr);
